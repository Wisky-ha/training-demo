"""Platform preprocessor for the prepared cooling-load dataset.
...
This script is the cooling-load counterpart of the electric-load
preprocessor.  The execution contract is identical; only the dataset and its
documentation differ.  Prediction requests intentionally omit the training
target, so ``transform`` supplies an empty placeholder when the target column
...
class Preprocessor:
    ...
    def fit(self, df, config):
        ...
        for column in df.columns:
            ...
            if not finite.any():
                # Entirely empty or non-numeric source features cannot provide
                # a trainable value and are excluded from the fitted schema.
                continue
    ...
    def transform(self, df, config):
        ...
        # A prediction frame has no training target.  Keep the target role
        # present for scripts that expect it without making it a required
        # prediction input.
        if self.target_column not in df.columns:
            working = df.copy(deep=True)
            working[self.target_column] = np.nan
        else:
            working = df

        output = pd.DataFrame(index=working.index)
        # Keep the positional platform roles explicit and unchanged.
        output[self.time_column] = working[self.time_column]
        for column in self.feature_columns:
            if column in working.columns:
                values = pd.to_numeric(working[column], errors="coerce")
            else:
                values = pd.Series(np.nan, index=working.index)
            values = values.replace([np.inf, -np.inf], np.nan)
            output[column] = values.fillna(self.fill_values[column]).astype(float)
        output[self.target_column] = working[self.target_column]
        return output
